        ...
        <div class="col-md-4">
            <a href="{{ route('inicio', ['ver' => 'pendientes']) }}" class="text-decoration-none">
                <div class="card border-0 shadow-sm rounded-4 p-4 border-start border-primary border-5 {{ $filtro == 'pendientes' ? 'bg-primary bg-opacity-10' : 'bg-white' }}">
                    ...
                    <h5 class="fw-bold text-dark mt-2 mb-0">Pendientes</h5>
                    <small class="text-muted">Por revisión inicial</small>
                </div>
            </a>
        </div>
        ...
        <div class="col-md-4">
            <a href="{{ route('inicio', ['ver' => 'enviadas']) }}" class="text-decoration-none">
                <div class="card border-0 shadow-sm rounded-4 p-4 border-start border-success border-5 {{ $filtro == 'enviadas' ? 'bg-success bg-opacity-10' : 'bg-white' }}">
                    ...
                    <h5 class="fw-bold text-dark mt-2 mb-0">Enviadas</h5>
                    <small class="text-muted">En proceso de viáticos</small>
                </div>
            </a>
        </div>
        ...
        <div class="col-md-4">
            <a href="{{ route('inicio', ['ver' => 'devueltas']) }}" class="text-decoration-none">
                <div class="card border-0 shadow-sm rounded-4 p-4 border-start border-danger border-5 {{ $filtro == 'devueltas' ? 'bg-danger bg-opacity-10' : 'bg-white' }}">
                    ...
                    <small class="text-muted">Con errores registrados</small>
                </div>
            </a>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="fw-bold text-dark text-uppercase fs-6">{{ $agenda->nombre_completo }}</div>
                            ...
                            <td class="text-center">
                                <button class="btn btn-dark btn-sm rounded-pill px-4 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#modalGestion{{ $agenda->id }}">
                                    @if(auth()->user()->role === 'coordinador') Revisar @else Ver Info @endif
                                </button>
                            </td>
                        </tr>
                        @empty
        ...
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header border-0 bg-light p-4">
                <h5 class="fw-bold mb-0">
                    {{ auth()->user()->role === 'coordinador' ? 'Gestión Técnica' : 'Detalles de la Agenda' }} #{{ $agenda->id }}
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>

            {{-- Solo el Coordinador tiene formulario --}}
            ...
            @endif
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="text-center mb-4">
                        ...
                        <p class="fs-4 fw-bold text-dark text-uppercase mb-0">{{ $agenda->nombre_completo }}</p>
                    </div>

                    {{-- Gestión de Observaciones --}}
                    @if(auth()->user()->role === 'coordinador')
                        <div class="mb-3">
                            <label class="form-label fw-bold text-dark">Observaciones de Revisión</label>
                            <textarea name="observaciones" class="form-control rounded-3" rows="4" placeholder="Indique motivos si decide devolver..." required>{{ $agenda->observaciones_finanzas }}</textarea>
                        </div>
                    @else
                        {{-- El Administrador solo lee si ya hay algo escrito --}}
                ...
                <div class="modal-footer border-0 p-4 pt-0">
                    @if(auth()->user()->role === 'coordinador')
                        <div class="d-flex justify-content-between w-100 gap-3">
                            <button type="submit" name="accion" value="devolver" class="btn btn-outline-danger rounded-pill px-4 fw-bold flex-grow-1">
                                <i class="fas fa-undo me-1"></i> Devolver
                            </button>
                            <button type="submit" name="accion" value="aprobar" class="btn btn-success rounded-pill px-4 fw-bold shadow-sm text-white flex-grow-1">
                                <i class="fas fa-check-circle me-1"></i> Validar
                            </button>
                        </div>
                    @else
                        <button type="button" class="btn btn-secondary w-100 rounded-pill fw-bold" data-bs-dismiss="modal">Cerrar Consulta</button>
                    @endif
                </div>
